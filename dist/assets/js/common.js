$(document).ready(function () {
    ...
    $(".desktop-menu li a, .navAdmin li a, .mobile-bottom-menu li a, .info li a").each(function () {
    ...
    if (urlPath == '/') {
        $(".desktop-menu ul li:first-child a").closest('li').addClass("active");
    }
    ...
    if ($('.s-cs').length) {
        ...
        $.each(entityData, function (i, item) {
            if (item.length > 0 && ['whatsapp', 'youtube', 'line', 'facebook', 'telegram', 'google', 'telephone', 'instagram', 'skype', 'twitter', 'yahoo', 'tiktok', 'sms', 'wechatId'].includes(i)) {
                ...
                $('.s-cs .cu').append(`
					<li class="`+ scName + `"><a href="` + link + `" target="_blank" class="flex flex-col items-center justify-center">` + faName + `<span>` + scName + `</span></a></li>
				`);

                $('.s-cs .cu2').append(`
					<li class="`+ scName + `"><a href="` + link + `" target="_blank" class="flex h-28 flex-col items-center justify-center bg-dandelion">` + faName + `<span>` + scName + `</span></a></li>
				`);
            }
        });
    }
